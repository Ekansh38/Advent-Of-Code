    ...
    for line in input.lines() {
        ...
        for (index, char) in line.chars().enumerate() {
            if char.is_digit(10) {
                ...
                for letter_num in letter_nums.iter() {
                    let letter_num_length = letter_num.len();
                    let mut count = 0;
                    let mut word: String = String::new();

                    for (new_index, new_char) in line.chars().enumerate() {
                        if new_index >= index && count < letter_num_length {
                            count += 1;
                            word.push(new_char);
                        }
                        if count == letter_num_length {
                            break;
                        }
                    }
                    if &word.as_str() == letter_num {
                        nums.push(into_int(letter_num));
                    }
                }
            }
        }
